...
- [Keys and shared signing fields](api/keys.md) — key lifecycle, policy replacement, legacy request fields, and common responses.
- [EVM](api/evm.md) — capability index for legacy and EIP-1559 transactions, EIP-712 Permit, ERC-4337, and EIP-7702.
- [TRON](api/tron.md) — supported signing routes and links to the detailed protobuf contract reference.
- [Verification and recovery](api/verification.md) — generic transaction-payload inspection; structured advanced-EVM inspection is documented with each [EVM capability](api/evm.md).
