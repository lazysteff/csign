...
func TestCapabilityWireFixtureUsesNestedTypedRecords(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(`{
		...
		"supported_tron_memo_capabilities":[{"encoding":"hex","max_transaction_bytes":512000,"signing_operations":["tron_transfer_trx","tron_transfer_trc20"]}]
	}`), &response))
	...
	require.Equal(t, TRONMemoEncodingHex, response.SupportedTRONMemoCapabilities[0].Encoding)
	require.Equal(t, TRONMaxTransactionBytes, response.SupportedTRONMemoCapabilities[0].MaxTransactionBytes)
	require.Equal(t, []string{OperationTRXTransfer, OperationTRC20Transfer}, response.SupportedTRONMemoCapabilities[0].SigningOperations)
}
